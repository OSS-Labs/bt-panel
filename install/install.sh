...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		wget -O install.sh https://raw.githubusercontent.com/oss-labs/bt-panel/master/install/install-ubuntu.sh && bash install.sh
		exit;
	else
		wget -O install.sh https://raw.githubusercontent.com/oss-labs/bt-panel/master/install/install-ubuntu.sh && sudo bash install.sh
		exit;
	fi
fi
...
echo "
+----------------------------------------------------------------------
|     适用于 CentOS/Redhat/Fedora/Ubuntu/Debian 的 Bt-WebPanel 5.x
+----------------------------------------------------------------------
|             Copyright © 2015-2018 BT-SOFT 版权所有
+----------------------------------------------------------------------
|             由开源软件实验室（OSSLab）提供技术支持
+----------------------------------------------------------------------
"
get_node_url(){
...
while [ "$go" != 'y' ] && [ "$go" != 'n' ]
do
	read -p "是否将 BT-Panel 安装到 $setup_path 目录？（y/n）" go;
done
...
EOF
			...
			if [ "$isN" = "" ];then
				echo '检测到 Windows 分区。为了您的数据安全，请手动卸载！';
				return;
			fi
...
Install_setuptools()
{
	...
	if [ ! -f "/usr/bin/easy_install" ];then
		echo '=================================================';
		echo -e "\033[31msetuptools 安装失败！\033[0m";
		exit;
	fi
}
...
Install_pip()
{
	...
	if [ "$ispip" = "" ];then
		echo '=================================================';
		echo -e "\033[31m Python-pip 安装失败！\033[0m";
		exit;
	fi
}
...
Install_Pillow()
{
	...
	if [ "$isSetup" = "" ];then
		echo '=================================================';
		echo -e "\033[31mPillow 安装失败！\033[0m";
		exit;
	fi
}
...
Install_psutil()
{
	...
	if [ "$isSetup" = "" ];then
		echo '=================================================';
		echo -e "\033[31mpsutil 安装失败！\033[0m";
		exit;
	fi
}
...
Install_chardet()
{
	...
	if [ "$isSetup" = "" ];then
		echo '=================================================';
		echo -e "\033[31mchardet 安装失败！\033[0m";
		exit;
	fi
}
...
Install_webpy()
{
	...
	if [ "$isSetup" = "" ];then
		echo '=================================================';
		echo -e "\033[31mweb.py 安装失败！\033[0m";
		exit;
	fi
}
...
wget -O panel.zip https://github.com/oss-labs/bt-panel/archive/master.zip -T 10
wget -O /etc/init.d/bt https://raw.githubusercontent.com/oss-labs/bt-panel/master/install/bt.init -T 10
...
mv -f $setup_path/server/bt-panel-master/* $setup_path/server/panel

rm -rf $setup_path/server/bt-panel-master
...
if [ ! -f $setup_path/server/panel/tools.py ];then
	echo -e "\033[31m安装包下载失败，请重试！\033[0m";
	echo '============================================'
	exit;
fi
...
echo -e "=================================================================="
echo -e "宝塔Linux面板：http://$address:$port"
echo -e "=================================================================="
echo -e "用 户 名：$username"
echo -e "密    码：$password"
echo -e ""
...
echo -e "总计耗时：\033[32m$outTime\033[0m 分钟"
rm -f install.sh
