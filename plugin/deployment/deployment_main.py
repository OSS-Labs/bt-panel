...
class deployment_main:
    ...
    def AddPackage(self, get):
        ...
        for d in data:
            if d['name'] == get.dname:
                return public.returnMsg(False, '您要添加的程序标识已存在！')
            if d['title'] == get.title:
                return public.returnMsg(False, '您要添加的程序名称已存在！')
    ...
    def DownloadFile(self, url, filename):
        try:
            ...
            urllib.urlretrieve(url, filename=filename,
                               reporthook=self.DownloadHook)
            self.WriteLogs(json.dumps(
                {'name': '下载文件', 'total': 0, 'used': 0, 'pre': 0, 'speed': 0}))
        except:
    ...
    def DownloadHook(self, count, blockSize, totalSize):
        ...
        if self.pre != pre1:
            ...
            speed = {'name': '下载文件', 'total': totalSize,
                     'used': used, 'pre': self.pre, 'speed': dspeed}
    ...
    def SetupPackage(self, get):
        ...
        find = public.M('sites').where(
            'name=?', (site_name,)).field('id,path').find()
        path = find['path']
        ...
        # 检查本地包
        self.WriteLogs(json.dumps(
            {'name': '检查软件包', 'total': 0, 'used': 0, 'pre': 0, 'speed': 0}))
        ...
        # 设置权限
        self.WriteLogs(json.dumps(
            {'name': '设置权限', 'total': 0, 'used': 0, 'pre': 0, 'speed': 0}))
        ...
        # 安装PHP扩展
        self.WriteLogs(json.dumps(
            {'name': '安装必要的PHP扩展', 'total': 0, 'used': 0, 'pre': 0, 'speed': 0}))
        if pinfo['ext'] != '':
        ...
        # 执行额外shell进行依赖安装
        self.WriteLogs(json.dumps(
            {'name': '执行额外SHELL', 'total': 0, 'used': 0, 'pre': 0, 'speed': 0}))
        ...
        if os.path.exists(path + '/composer.json'):
            ...
            if not os.path.exists(path + '/composer.lock'):
                ...
                if execPHP:
                    ...
                    os.system('nohup cd '+path+' && '+execPHP +
                              ' /usr/bin/composer install -vvv > /tmp/composer.log 2>&1 &')

        # 写伪静态
        self.WriteLogs(json.dumps(
            {'name': '设置伪静态', 'total': 0, 'used': 0, 'pre': 0, 'speed': 0}))
        ...
        # 设置运行目录
        self.WriteLogs(json.dumps(
            {'name': '设置运行目录', 'total': 0, 'used': 0, 'pre': 0, 'speed': 0}))
        if pinfo['run'] != '/':
            import panelSite
        ...
        # 导入数据
        self.WriteLogs(json.dumps(
            {'name': '导入数据库', 'total': 0, 'used': 0, 'pre': 0, 'speed': 0}))
        if os.path.exists(path+'/import.sql'):
            databaseInfo = public.M('databases').where(
                'pid=?', (find['id'],)).field('username,password').find()
            if databaseInfo:
                os.system('/www/server/mysql/bin/mysql -u' + databaseInfo['username'] + ' -p' +
                          databaseInfo['password'] + ' ' + databaseInfo['username'] + ' < ' + path + '/import.sql')
        ...
        self.depTotal(name)
        self.WriteLogs(json.dumps(
            {'name': '准备部署', 'total': 0, 'used': 0, 'pre': 0, 'speed': 0}))
    ...
    def depTotal(self, name):
        try:
            import urllib2
            urllib2.urlopen(
                "https://www.bt.cn/Api/depTotal?name=" + name, timeout=3)
            return True
        except:
            return False
