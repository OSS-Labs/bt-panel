...
class panelPluginApi(common.panelSetup):
    def GET(self):
        get = web.input();
        if not public.checkToken(get): return public.returnJson(False,'无效的 Token ！');
...
class panelToken(common.panelSetup):
    def GET(self):
        ...
        if not os.path.exists(self.tokenFile): return json.dumps(public.returnMsg(False,'当前未开启 API 接口服务！'));
