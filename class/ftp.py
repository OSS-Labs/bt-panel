...
class ftp:
    ...
    def AddUser(self, get):
        try:
            ...
            if hasattr(get, 'pid'):
                pid = get.pid
            public.M('ftps').add('pid,name,password,path,status,ps,addtime',
    ...
    def SetUserPassword(self, get):
        try:
            ...
            public.ExecShell(self.__runPath + '/pure-pw passwd ' +
                             username + '<<EOF \n' + password + '\n' + password + '\nEOF')
            self.FtpReload()
            public.M('ftps').where("id=?", (id,)).setField(
                'password', password)
    ...
    def SetStatus(self, get):
        ...
        try:
            ...
            status = get['status']
            if int(status) == 0:
                public.ExecShell(self.__runPath +
                                 '/pure-pw usermod ' + username + ' -r 1')
            else:
                public.ExecShell(self.__runPath +
                                 '/pure-pw usermod ' + username + " -r ''")
            self.FtpReload()
