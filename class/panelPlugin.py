...
class panelPlugin:
    ...
    def install(self, get):
        ...
        if not pluginInfo:
            ...
            pluginInfo = json.loads(public.readFile(
                self.__install_path + '/' + get.name + '/info.json'))
        ...
        if pluginInfo['tip'] == 'lib':
            ...
            if self.checksSetup(pluginInfo['name'], pluginInfo['checks'], pluginInfo['versions'])[0]['status'] or os.path.exists(self.__install_path + '/' + get.name):
                public.WriteLog(
                    'TYPE_SETUP', 'PLUGIN_INSTALL_LIB', (pluginInfo['title'],))
            ...
            execstr = "cd /www/server/panel/install && /bin/bash install_soft.sh " + \
                get.type + " "+mtype+" " + get.name + " " + get.version
            sql = db.Sql()
            if hasattr(get, 'id'):
                id = get.id
            else:
                id = None
            sql.table('tasks').add('id,name,type,status,addtime,execstr', (None, mmsg +
                                                                           '['+get.name+'-'+get.version+']', 'execshell', '0', time.strftime('%Y-%m-%d %H:%M:%S'), execstr))
    ...
    def getPluginList(self, get):
        ...
        for i in range(len(arr)):
            ...
            if 'price' in arr[i]:
                if arr[i]['price'] > 0:
                    ...
                    if os.path.exists('plugin/beta/config.conf'):
                        if os.path.exists('/www/server/panel/plugin/' + arr[i]['name'] + '/' + arr[i]['name'] + '_main.py') and arr[i]['end'] == '未开通':
                            arr[i]['end'] = '--'

            if arr[i]['name'] == 'php':
                if apacheVersion == '2.2':
                    arr[i]['versions'] = '5.2,5.3,5.4'
                    arr[i]['update'] = self.GetPv(
                        arr[i]['versions'], arr[i]['update'])
            ...
            arr[i]['versions'] = self.checksSetup(arr[i]['name'].replace(
                '_soft', ''), arr[i]['checks'], arr[i]['versions'])
            ...
            if arr[i]['tip'] == 'lib':
                arr[i]['path'] = self.__install_path + \
                    '/' + arr[i]['name'].replace('_soft', '')
                arr[i]['config'] = os.path.exists(
                    arr[i]['path'] + '/index.html')
            else:
                arr[i]['path'] = '/www/server/' + \
                    arr[i]['name'].replace('_soft', '')
        ...
        for v in versArr:
            ...
            for tm in tmp:
                if name == 'php':
                    ...
                    if os.path.exists(path + '/bin/php') and not os.path.exists(path + '/version.pl'):
                        public.ExecShell(
                            "echo `"+path+"/bin/php 2>/dev/null -v|grep cli|awk '{print $2}'` > " + path + '/version.pl')
                    try:
            ...
            if name == 'apache':
                if os.path.exists('/www/server/apache/bin/httpd'):
                    ...
                    if not v1:
                        v1 = public.ExecShell(
                            "/www/server/apache/bin/httpd -v|grep Apache|awk '{print $3}'|sed 's/Apache\///'")[0].strip()
    ...
    def getPHPMyAdminStatus(self):
        ...
        if os.path.exists(configFile):
            ...
            if not os.path.exists(configFile):
                public.writeFile(configFile, public.readFile(
    ...
    def a(self, get):
        ...
        if not hasattr(pluginObject, get.s):
