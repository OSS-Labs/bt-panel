...
class panelSSL:
    ...
    def GetUserInfo(self, get):
        ...
        if self.__userInfo:
            userTmp = {}
            userTmp['username'] = self.__userInfo['username'][0:3] + \
                '****'+self.__userInfo['username'][-4:]
            result['status'] = True
    ...
    def GetOrderList(self, get):
        ...
        result = json.loads(public.httpPost(
            self.__APIURL + '/GetSSLList', self.__PDATA))
        result['data'] = self.En_Code(result['data'])
    ...
    def GetDVSSL(self, get):
        get.id = public.M('domain').where(
            'name=?', (get.domain,)).getField('pid')
        ...
        self.__PDATA['data'] = self.De_Code(self.__PDATA['data'])
        result = json.loads(public.httpPost(
            self.__APIURL + '/GetDVSSL', self.__PDATA))
        result['data'] = self.En_Code(result['data'])
    ...
    def CheckDomain(self, get):
        try:
            ...
            result = public.httpGet(
                'http://' + get.domain + '/.well-known/pki-validation/fileauth.txt')
            if result == epass:
                return True
            return False
        except:
    ...
    def Completed(self, get):
        ...
        if hasattr(get, 'siteName'):
            ...
            if runPath != False and runPath != '/':
                get.path += runPath
            sslInfo = json.loads(public.httpPost(
                self.__APIURL + '/SyncOrder', self.__PDATA))
            sslInfo['data'] = self.En_Code(sslInfo['data'])
            try:
                spath = get.path + '/.well-known/pki-validation'
                if not os.path.exists(spath):
                    os.system("mkdir -p '" + spath + "'")
                public.writeFile(spath + '/fileauth.txt',
                                 sslInfo['data']['authValue'])
            except:
                return public.returnMsg(False, 'SSL_CHECK_WRITE_ERR')
        result = json.loads(public.httpPost(
            self.__APIURL + '/Completed', self.__PDATA))
        result['data'] = self.En_Code(result['data'])
        return result
    ...
    def GetSSLInfo(self, get):
        ...
        self.__PDATA['data'] = self.De_Code(self.__PDATA['data'])
        result = json.loads(public.httpPost(
            self.__APIURL + '/GetSSLInfo', self.__PDATA))
        result['data'] = self.En_Code(result['data'])
    ...
    def GetCertName(self, get):
        try:
            ...
            result = public.ExecShell(
                openssl + " x509 -in "+get.certPath+" -noout -subject -enddate -startdate -issuer")
            tmp = result[0].split("\n")
            data = {}
            ...
            result = public.ExecShell(
                openssl + " x509 -in "+get.certPath+" -noout -text|grep DNS")
    ...
    def GetSSLProduct(self, get):
        self.__PDATA['data'] = self.De_Code(self.__PDATA['data'])
        result = json.loads(public.httpPost(
            self.__APIURL + '/GetSSLProduct', self.__PDATA))
        result['data'] = self.En_Code(result['data'])
        return result
