...
class config:
    ...
    def setPassword(self, get):
        if get.password1 != get.password2:
            return public.returnMsg(False, 'USER_PASSWORD_CHECK')
        if len(get.password1) < 5:
            return public.returnMsg(False, 'USER_PASSWORD_LEN')
        public.M('users').where("username=?", (web.ctx.session.username,)).setField(
            'password', public.md5(get.password1.strip()))
        public.WriteLog('TYPE_PANEL', 'USER_PASSWORD_SUCCESS',
    ...
    def setUsername(self, get):
        if get.username1 != get.username2:
            return public.returnMsg(False, 'USER_USERNAME_CHECK')
        if len(get.username1) < 3:
            return public.returnMsg(False, 'USER_USERNAME_LEN')
        public.M('users').where("username=?", (web.ctx.session.username,)).setField(
            'username', get.username1.strip())
        public.WriteLog('TYPE_PANEL', 'USER_USERNAME_SUCCESS',
                        (web.ctx.session.username, get.username2))
    ...
    def setPanel(self, get):
        ...
        if get.domain.strip():
            mhost = get.domain
        data = {'uri': web.ctx.fullpath, 'host': mhost+':'+newPort,
                'status': True, 'isReWeb': isReWeb, 'msg': public.getMsg('PANEL_SAVE')}
        public.WriteLog('TYPE_PANEL', 'PANEL_SAVE', (newPort, get.domain,
                                                     get.backup_path, get.sites_path, get.address, get.limitip))
        self.setService()
        if isReWeb:
    ...
    def setService(self):
        ...
        if not os.path.exists(panel_path + '/service/sbin/BT-Panel'):
            return True
...
''' % (port, domain, myssl)
    ...
    def setPHPMaxSize(self, get):
        ...
        if int(max) < 2:
            return public.returnMsg(False, 'PHP_UPLOAD_MAX_ERR')
    ...
    def syncDate(self, get):
        dateStr = public.httpGet(
            web.ctx.session['home'] + '/api/index/get_date')
    ...
    def SetControl(self, get):
        try:
            if hasattr(get, 'day'):
                ...
                get.day = str(get.day)
                if(get.day < 1):
                    return public.returnMsg(False, "CONTROL_ERR")
        except:
            pass
  ...
  `addtime` INTEGER
    ...
    def AddPanelInfo(self, get):
        ...
        isAdd = public.M('panel').where(
            'title=? OR url=?', (get.title, get.url)).count()
        if isAdd:
            return public.returnMsg(False, 'PANEL_SSL_ADD_EXISTS')
        import time
        import json
        isRe = public.M('panel').add('title,url,username,password,click,addtime',
                                     (get.title, get.url, get.username, get.password, 0, int(time.time())))
        if isRe:
    ...
    def SetPanelInfo(self, get):
        ...
        isSave = public.M('panel').where(
            '(title=? OR url=?) AND id!=?', (get.title, get.url, get.id)).count()
        if isSave:
            return public.returnMsg(False, 'PANEL_SSL_ADD_EXISTS')
        import time
        import json
        ...
        # 更新到数据库
        isRe = public.M('panel').where('id=?', (get.id,)).save(
            'title,url,username,password', (get.title, get.url, get.username, get.password))
        if isRe:
    ...
    def DelPanelInfo(self, get):
        ...
        if not isExists:
            return public.returnMsg(False, 'PANEL_SSL_ADD_NOT_EXISTS')
    ...
    def SetPHPConf(self, get):
        gets = ['display_errors', 'cgi.fix_pathinfo', 'date.timezone', 'short_open_tag', 'asp_tags', 'max_execution_time', 'max_input_time',
                'memory_limit', 'post_max_size', 'file_uploads', 'upload_max_filesize', 'max_file_uploads', 'default_socket_timeout', 'error_reporting']
    ...
    def SavePanelSSL(self, get):
        ...
        if not public.CheckCert(checkCert):
            return public.returnMsg(False, '证书错误！')
