...
class files:
    ...
    def CreateDir(self, get):
        ...
        try:
            if not self.CheckFileName(get.path):
                return public.returnMsg(False, '目录名不能包含特殊字符！')
            if os.path.exists(get.path):
    ...
    def DeleteDir(self, get):
        get.path = get.path.encode('utf-8')
        # if get.path.find('/www/wwwroot') == -1: return public.returnMsg(False,'演示服务器，禁止删除此目录！');
        if not os.path.exists(get.path):
    ...
    def GetFileBody(self, get):
        ...
        if os.path.getsize(get.path) > 2097152:
            return public.returnMsg(False, '不能在线编辑大于 2MB 的文件！')
        ...
        try:
            ...
            if isConf != -1:
                ...
                if isError != True:
                    ...
                    return public.returnMsg(False, '<font style="color:red;">'+isError.replace("\n", '<br>')+'</font>')
    ...
    def Zip(self, get):
        ...
        try:
            ...
            if get.type == 'zip':
                os.system("cd '"+get.path+"' && zip '"+get.dfile +
                          "' -r '"+get.sfile+"' > "+tmps+" 2>&1")
            else:
                sfiles = ''
                for sfile in get.sfile.split(','):
                    if not sfile:
                        continue
                    sfiles += " '" + sfile + "'"
                os.system("cd '" + get.path + "' && tar -zcvf '" +
                          get.dfile + "' " + sfiles + " > " + tmps + " 2>&1")
    ...
    def UnZip(self, get):
        ...
        # try:
        if not hasattr(get, 'coding'):
            get.coding = 'UTF-8'
        tmps = '/tmp/panelExec.log'
        if get.sfile[-4:] == '.zip':
            os.system("export LANG=\"zh_CN." + get.coding + "\" && unzip -P '"+get.password +
                      "' -o '" + get.sfile + "' -d '" + get.dfile + "' > " + tmps + " 2>&1")
        elif get.sfile[-7:] == '.tar.gz' or get.sfile[-4:] == '.tgz':
            os.system("tar zxf '" + get.sfile + "' -C '" +
                      get.dfile + "' > " + tmps + " 2>&1")
        else:
    ...
    def CloseLogs(self, get):
        ...
        if public.get_webserver() == 'nginx':
            os.system('kill -USR1 `cat '+web.ctx.session.setupPath +
    ...
    def BatchPaste(self, get):
        ...
        if get.type == '1':
            for key in web.ctx.session.selected.data:
                ...
                public.writeSpeed(key, i, l)
                try:
                    sfile = web.ctx.session.selected.path + \
                        '/' + key.encode('utf-8')
            ...
            public.WriteLog('TYPE_FILE', 'FILE_ALL_COPY',
                            (web.ctx.session.selected.path, get.path))
        else:
            for key in web.ctx.session.selected.data:
                try:
                    i += 1
    ...
    def DownloadFile(self, get):
        ...
        sql = db.Sql()
        sql.table('tasks').add('name,type,status,addtime,execstr',
                               ('下载文件['+get.filename+']', 'download', '0', time.strftime('%Y-%m-%d %H:%M:%S'), execstr))
    ...
    def InstallSoft(self, get):
        ...
        if public.get_webserver() == 'apache':
            apacheVersion = public.readFile(
                web.ctx.session.setupPath+'/apache/version.pl')
        public.writeFile('/var/bt_apacheVersion.pl', apacheVersion)
        public.writeFile('/var/bt_setupPath.conf', web.ctx.session.rootPath)
        isTask = '/tmp/panelTask.pl'
        execstr = "cd " + web.ctx.session.setupPath + "/panel/install && /bin/bash install_soft.sh " + \
...
do
    ...
    def GetTaskSpeed(self, get):
        ...
        if not len(find):
            return public.returnMsg(False, '当前没有任务队列在执行！')
        isTask = '/tmp/panelTask.pl'
        public.writeFile(isTask, 'True')
        echoMsg = {}
