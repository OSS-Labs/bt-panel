...
class database:
    ...
    def AddDatabase(self, get):
        try:
            ...
            if sql.where("name=? or username=?", (data_name, username)).count():
            ...
            # 添加MYSQL
            result = panelMysql.panelMysql().execute("create database `" + data_name +
            ...
            panelMysql.panelMysql().execute("grant all privileges on `" + data_name +
                                            "`.* to '" + username + "'@'localhost' identified by '" + data_pwd + "'")
            for a in address.split(','):
                panelMysql.panelMysql().execute("grant all privileges on `" + data_name +
                                                "`.* to '" + username + "'@'" + a + "' identified by '" + data_pwd + "'")
            ...
            if hasattr(get, 'pid'):
                pid = get.pid
            # 添加入SQLITE
            sql.add('pid,name,username,password,accept,ps,addtime', (pid,
                                                                     data_name, username, password, address, get['ps'], addTime))
            public.WriteLog("TYPE_DATABASE",
                            'DATABASE_ADD_SUCCESS', (data_name,))
    ...
    def IsSqlError(self, mysqlMsg):
        ...
        if "MySQLdb" in mysqlMsg:
            return public.returnMsg(False, 'DATABASE_ERR_MYSQLDB')
        if "2002," in mysqlMsg:
            return public.returnMsg(False, 'DATABASE_ERR_CONNECT')
        if "using password:" in mysqlMsg:
            return public.returnMsg(False, 'DATABASE_ERR_PASS')
        if "Connection refused" in mysqlMsg:
            return public.returnMsg(False, 'DATABASE_ERR_CONNECT')
        if "1133" in mysqlMsg:
            return public.returnMsg(False, 'DATABASE_ERR_NOT_EXISTS')
        return None
    ...
    def DeleteDatabase(self, get):
        try:
            ...
            if isError != None:
                return isError
            users = panelMysql.panelMysql().query("select Host from mysql.user where User='" +
            ...
            public.WriteLog("TYPE_DATABASE", 'DATABASE_DEL_ERR',
                            (get.name, str(ex)))
    ...
    def DeleteToRecycleBin(self, name):
        ...
        public.writeFile(rPath + 'BTDB_' + name + '_t_' +
                         str(time.time()), json.dumps(data))
    ...
    def DeleteTo(self, filename):
        ...
        if isError != None:
            return isError
        panelMysql.panelMysql().execute(
            "drop user '" + data['username'] + "'@'localhost'")
        users = panelMysql.panelMysql().query("select Host from mysql.user where User='" +
                                              data['username'] + "' AND Host!='localhost'")
        for us in users:
            panelMysql.panelMysql().execute(
                "drop user '" + data['username'] + "'@'" + us[0] + "'")
        panelMysql.panelMysql().execute("flush privileges")
        os.remove(filename)
        public.WriteLog("TYPE_DATABASE",
                        'DATABASE_DEL_SUCCESS', (data['name'],))
        return public.returnMsg(True, 'DEL_SUCCESS')
    ...
    def RecycleDB(self, filename):
        ...
        result = panelMysql.panelMysql().execute("grant all privileges on `" +
                                                 data['name'] + "`.* to '" + data['username'] + "'@'localhost' identified by '" + data['password'] + "'")
        isError = self.IsSqlError(result)
        if isError != None:
            return isError
        panelMysql.panelMysql().execute("grant all privileges on `" +
                                        data['name'] + "`.* to '" + data['username'] + "'@'" + data['accept'] + "' identified by '" + data['password'] + "'")
        panelMysql.panelMysql().execute("flush privileges")
...
echo "root密码成功修改为: ${pwd}"
    ...
    def ResDatabasePassword(self, get):
        try:
            ...
            # 修改MYSQL
            if '5.7' in public.readFile(web.ctx.session.setupPath + '/mysql/version.pl'):
                result = panelMysql.panelMysql().execute("update mysql.user set authentication_string=password('" +
                                                         newpassword + "') where User='" + username + "'")
            else:
                result = panelMysql.panelMysql().execute("update mysql.user set Password=password('" +
                                                         newpassword + "') where User='" + username + "'")
            ...
            # 修改SQLITE
            if int(id) > 0:
                public.M('databases').where("id=?", (id,)
                                            ).setField('password', newpassword)
            else:
                public.M('config').where("id=?", (id,)).setField(
                    'mysql_root', newpassword)
            ...
            public.WriteLog("TYPE_DATABASE",
                            'DATABASE_PASS_ERROR', (name, str(ex)))
    ...
    def ToBackup(self, get):
        ...
        if not os.path.exists(web.ctx.session.config['backup_path'] + '/database'):
            os.system('mkdir -p ' +
                      web.ctx.session.config['backup_path'] + '/database')
        self.mypass(True, root)

        fileName = name + '_' + \
            time.strftime('%Y%m%d_%H%M%S', time.localtime()) + '.sql.gz'
        backupName = web.ctx.session.config['backup_path'] + \
            '/database/' + fileName
        public.ExecShell(
            "/www/server/mysql/bin/mysqldump --force --opt " + name + " | gzip > " + backupName)
    ...
    def DelBackup(self, get):
        try:
            ...
            filename = public.M('backup').where(
                where, (id,)).getField('filename')
            if os.path.exists(filename):
                os.remove(filename)
            name = ''
            if filename == 'qiniu':
                name = public.M('backup').where(where, (id,)).getField('name')
                public.ExecShell("python "+web.ctx.session.setupPath +
                                 '/panel/script/backup_qiniu.py delete_file ' + name)

            public.M('backup').where(where, (id,)).delete()
            public.WriteLog("TYPE_DATABASE",
                            'DATABASE_BACKUP_DEL_SUCCESS', (name, filename))
            return public.returnMsg(True, 'DEL_SUCCESS')
        except Exception, ex:
            public.WriteLog(
                "TYPE_DATABASE", 'DATABASE_BACKUP_DEL_ERR', (name, filename, str(ex)))
            return public.returnMsg(False, 'DEL_ERROR')
    ...
    def InputSql(self, get):
        try:
            ...
            if ext != 'sql':
                ...
                self.mypass(True, root)
                public.ExecShell(web.ctx.session.setupPath + "/mysql/bin/mysql -uroot -p" +
            ...
            public.WriteLog("TYPE_DATABASE",
                            'DATABASE_INPUT_ERR', (name, str(ex)))
    ...
    def ToDataBase(self, find):
        ...
        if len(find['password']) < 3:
            ...
            find['password'] = public.md5(
                str(time.time()) + find['name'])[0:10]
            public.M('databases').where("id=?", (find['id'],)).save(
                'password,username', (find['password'], find['username']))

        result = panelMysql.panelMysql().execute(
            "create database " + find['name'])
        if "using password:" in str(result):
            return -1
        if "Connection refused" in str(result):
            return -1
        panelMysql.panelMysql().execute(
            "drop user '" + find['username'] + "'@'localhost'")
        panelMysql.panelMysql().execute(
            "drop user '" + find['username'] + "'@'" + find['accept'] + "'")
        password = find['password']
        if find['password'] != "" and len(find['password']) > 20:
            password = find['password']

        panelMysql.panelMysql().execute("grant all privileges on " +
                                        find['name'] + ".* to '" + find['username'] + "'@'localhost' identified by '" + password + "'")
        panelMysql.panelMysql().execute("grant all privileges on " +
                                        find['name'] + ".* to '" + find['username'] + "'@'" + find['accept'] + "' identified by '" + password + "'")
        panelMysql.panelMysql().execute("flush privileges")
        return 1
    ...
    def SyncGetDatabases(self, get):
        ...
        for value in data:
            ...
            addTime = time.strftime('%Y-%m-%d %X', time.localtime())
            if sql.table('databases').add('name,username,password,accept,ps,addtime', (value[0], value[0], '', host, ps, addTime)):
                n += 1
    ...
    def SetDatabaseAccess(self, get):
        # try:
        name = get['name']
        db_name = public.M('databases').where(
            'username=?', (name,)).getField('name')
        access = get['access']
        ...
        password = public.M('databases').where(
            "username=?", (name,)).getField('password')
        users = panelMysql.panelMysql().query(
            "select Host from mysql.user where User='" + name + "' AND Host!='localhost'")
        for us in users:
            panelMysql.panelMysql().execute(
        ...
        for a in access.split(','):
            panelMysql.panelMysql().execute("grant all privileges on " + db_name +
                                            ".* to '" + name + "'@'" + a + "' identified by '" + password + "'")
        panelMysql.panelMysql().execute("flush privileges")
    ...
    def BinLog(self, get):
        ...
        if mycnf.find('#log-bin=mysql-bin') != -1:
            ...
            mycnf = mycnf.replace('#binlog_format=mixed',
                                  'binlog_format=mixed')
            os.system('sync')
            os.system('/etc/init.d/mysqld restart')
        else:
