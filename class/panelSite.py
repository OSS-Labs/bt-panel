...
class panelSite:
    ...
    def apacheAdd(self):
        ...
        try:
            httpdVersion = public.readFile(
                self.setupPath+'/apache/version.pl').strip()
        except:
            httpdVersion = ""
        if httpdVersion == '2.2':
            vName = ''
    ...
    <Directory "%s">
        ...
        if not os.path.exists(self.sitePath+'/.htaccess'):
            public.writeFile(self.sitePath+'/.htaccess', ' ')
        ...
        conf = '''server
{
    ...
    def AddSite(self, get):
        ...
        if isError != True:
            return public.returnMsg(False, '检测到配置文件有错误，请先排除后再操作。<br><a style="color:red;">'+isError.replace("\n", '<br>')+'</a>')
        ...
        # 表单验证
        if not files.files().CheckDir(self.sitePath):
        ...
        if not os.path.exists(userIni):
            public.writeFile(userIni, 'open_basedir=' +
                             self.sitePath+'/:/tmp/:/proc/')
        ...
        # 写入数据库
        get.pid = sql.table('sites').add('name,path,status,ps,addtime',
                                         (self.siteName, self.sitePath, '1', ps, public.getDate()))
    ...
    def DeleteSite(self, get):
        ...
        if os.path.exists(filename):
            os.remove(filename)
            public.ExecShell("rm -f " + confPath +
                             '/rewrite/' + siteName + "_*")
    ...
    def AddDomain(self, get):
        ...
        isError = public.checkWebConfig()
        if isError != True:
            return public.returnMsg(False, '检测到配置文件有错误，请先排除后再操作。<br><br><a style="color:red;">'+isError.replace("\n", '<br>')+'</a>')
        ...
        for domain in domains:
            ...
            # 检查域名是否存在
            sql = public.M('domain')
            opid = sql.where("name=? AND (port=? OR pid=?)",
                             (get.domain, get.port, get.id)).getField('pid')
            if opid:
                if public.M('sites').where('id=?', (opid,)).count():
            ...
            public.serviceReload()
            public.WriteLog('TYPE_SITE', 'DOMAIN_ADD_SUCCESS',
    ...
    def ApacheDomain(self, get):
        ...
        find = public.M('sites').where(
            "id=?", (get.id,)).field('id,name,path').find()
        sitePath = find['path']
    ...
    def DelDomain(self, get):
        ...
        id = get['id']
        port = get.port
        find = sql.where("pid=? AND name=?", (get.id, get.domain)).field(
            'id,name').find()
        ...
        if conf:
            ...
            rep = "listen\s+([0-9]+);"
            tmp = re.findall(rep, conf)
            port_count = sql.table('domain').where(
                'pid=? AND port=?', (get.id, get.port)).count()
        ...
        public.WriteLog('TYPE_SITE', 'DOMAIN_DEL_SUCCESS',
                        (get.webname, get.domain))
        public.serviceReload()
    ...
    def SetSSL(self, get):
        ...
        if(type(isError) == str):
            ...
            return public.returnMsg(False, '<a style="color:red;">'+isError.replace("\n", '<br>')+'</a>')
        public.serviceReload()
    ...
    def GetRunPath(self, get):
        if hasattr(get, 'siteName'):
            get.id = public.M('sites').where(
                'name=?', (get.siteName,)).getField('id')
        else:
            get.id = public.M('sites').where(
                'path=?', (get.path,)).getField('id')
        if not get.id:
            return False
        import panelSite
    ...
    def CreateLet(self, get):
        ...
        siteInfo = public.M('sites').where(
            'name=?', (get.siteName,)).field('id,name,path').find()
        ...
        if not os.path.exists(acem):
            try:
                public.ExecShell("curl -sS "+public.get_url() +
                                 "/install/acme_install.sh|bash")
            except:
                public.ExecShell(
                    "curl -sS http://download.bt.cn/install/acme_install.sh|bash")
        if not os.path.exists(acem):
        ...
        if hasattr(get, 'renew'):
            ...
            if hasattr(get, 'dnsapi'):
                if get.dnsapi == 'dns':
                    ...
                    execStr += ' --dns ' + get.dnsapi + \
                        ' --dnssleep ' + str(get.dnssleep)
                    if not self.Check_DnsApi(get.dnsapi):
                        return public.returnMsg(False, '请先设置该API！')
                    if get.dnsapi == 'dns_bt':
                        c_file = '/www/server/panel/plugin/dns/dns_main.py'
                        if not os.path.exists(c_file):
                            return public.returnMsg(False, '请先安装[云解析]插件！')
                        c_conf = public.readFile(c_file)
                        if c_conf.find('add_txt') == -1:
                            os.system(
                                'wget -O '+filename+' http://download.bt.cn/install/plugin/dns/dns_main.py -T 5')
        ...
        for domain in domains:
            ...
            get.path = p
            if force:
                if not self.CheckDomainPing(get):
                    errorDomain += '<li>' + domain + '</li>'
            if dns_plu:
                domainId, key = dns_plu.get_domainid_byfull('test.' + domain)
                if not domainId:
                    errorDns += '<li>' + domain + '</li>'
            if p != done:
                done = p
                execStr += ' -w ' + done
            execStr += ' -d ' + domain
        ...
        if errorDomain:
            return public.returnMsg(False, 'SITE_SSL_ERR_DNS', ('<span style="color:red;"><br>'+errorDomain+'</span>',))
        if errorDns:
            return public.returnMsg(False, '以下域名不在[云解析]插件中：%s' % '<span style="color:red;"><br>'+errorDns+'</span>')
        # 获取域名数据
        if domainCount == 0:
            return public.returnMsg(False, 'SITE_SSL_ERR_EMPTY')
        ...
        result = public.ExecShell(
            'export ACCOUNT_EMAIL=' + email + ' && ' + execStr)
        ...
        if dns and not os.path.exists(home_cert):
            try:
                ...
                data['txtValue'] = re.findall(
                    "TXT\s+value:\s*'(.+)'", result[0])
                return data
            except:
                data = {}
                data['err'] = result
        ...
        if not os.path.exists(home_cert):
            ...
            if result[1].find('new-authz error:') != -1:
                ...
                if data['result']['status'] == 429:
                    data['msg'] = '签发失败，您尝试申请证书的失败次数已达上限！<p>1、检查域名是否绑定到对应站点</p><p>2、检查域名是否正确解析到本服务器，或解析还未完全生效</p><p>3、如果您的站点设置了反向代理，或使用了CDN，请先将其关闭</p><p>4、如果您的站点设置了301重定向，请先将其关闭</p><p>5、如果以上检查都确认没有问题，请尝试更换DNS服务商</p>'
            data['status'] = False
            return data

        if not os.path.exists(path):
            public.ExecShell("mkdir -p " + path)
        public.ExecShell("ln -sf " + home_cert.replace("*",
                                                       "\*") + " " + csrpath.replace("*", "\*"))
        public.ExecShell("ln -sf " + home_key.replace("*",
                                                      "\*") + " " + keypath.replace("*", "\*"))
        public.ExecShell('echo "let" > ' + path + '/README')
        if(actionstr == '2'):
            return public.returnMsg(True, 'SITE_SSL_UPDATE_SUCCESS')
    ...
    def GetDnsApi(self, get):
        ...
        if not os.path.exists(path + '/dnsapi'):
            os.makedirs(path + '/dnsapi')
        ...
        for i in range(len(apis)):
            ...
            if not os.path.exists(filename) and apis[i]['name'] != 'dns':
                public.downloadFile(
                    'http://download.bt.cn/install/dnsapi/' + apis[i]['name'] + '.sh', filename)
                public.ExecShell("chmod +x " + filename)
            if not apis[i]['data']:
                continue
            for j in range(len(apis[i]['data'])):
                match = re.search(apis[i]['data'][j]
                                  ['key'] + "\s*=\s*'(.+)'", account)
                if match:
    ...
    def GetSiteDomains(self, get):
        ...
        domains = public.M('domain').where(
            'pid=?', (get.id,)).field('name,id').select()
        binding = public.M('binding').where(
            'pid=?', (get.id,)).field('domain,id').select()
        if type(binding) == str:
            return binding
        for b in binding:
            tmp = {}
...
""" % (siteName, siteName)
                ...
                if not public.inArray(tmp, '443'):
                    listen = re.search(rep, conf).group()
                    versionStr = public.readFile(
                        '/www/server/nginx/version.pl')
                    http2 = ''
                    if versionStr:
        ...
        if conf:
            if conf.find('SSLCertificateFile') == -1:
                find = public.M('sites').where(
                    "name=?", (siteName,)).field('id,path').find()
                tmp = public.M('domain').where(
                    'pid=?', (find['id'],)).field('name').select()
                domains = ''
                for key in tmp:
                    domains += key['name'] + ' '
                path = (find['path'] + '/' +
                        self.GetRunPath(get)).replace('//', '/')
    ...
    def GetSSL(self, get):
        ...
        file = self.setupPath + '/panel/vhost/' + \
            public.get_webserver() + '/'+siteName+'.conf'
        conf = public.readFile(file)
    ...
    def SiteStop(self, get):
        ...
        if not os.path.exists(path):
            os.makedirs(path)
            public.downloadFile(
                'http://download.bt.cn/stop.html', path + '/index.html')

        binding = public.M('binding').where('pid=?', (id,)).field(
        ...
        try:
            if(public.get_webserver() == 'nginx'):
                conf = public.readFile(
                    self.setupPath + '/panel/vhost/nginx/' + siteName + '.conf')
                ...
                conf = public.readFile(
                    self.setupPath + '/panel/vhost/apache/' + siteName + '.conf')
                if conf.find('301-START') == -1:
                    result['domain'] = domains[:-1]
                    result['src'] = ""
                    result['status'] = False
    ...
    def AddDirBinding(self, get):
        ...
        filename = self.setupPath + '/panel/vhost/nginx/' + \
            siteInfo['name'] + '.conf'
        conf = public.readFile(filename)
        if conf:
...
#BINDING-%s-END''' % (domain, port, domain, webdir, version, self.setupPath, siteInfo['name'], web.ctx.session.logsPath+'/'+siteInfo['name'], web.ctx.session.logsPath+'/'+siteInfo['name'], domain)
        ...
        filename = self.setupPath + '/panel/vhost/apache/' + \
            siteInfo['name'] + '.conf'
        conf = public.readFile(filename)
        if conf:
            try:
...
#BINDING-%s-END''' % (domain, port, webdir, domain, web.ctx.session.logsPath+'/'+siteInfo['name'], web.ctx.session.logsPath+'/'+siteInfo['name'], phpConfig, webdir, apaOpt, domain)
    ...
    def DelDirBinding(self, get):
        id = get.id
        binding = public.M('binding').where(
            "id=?", (id,)).field('id,pid,domain,path').find()
        siteName = public.M('sites').where(
        ...
        filename = self.setupPath + '/panel/vhost/rewrite/' + \
            siteName + '_' + binding['path'] + '.conf'
        if os.path.exists(filename):
            os.remove(filename)
        public.serviceReload()
        public.WriteLog('TYPE_SITE', 'SITE_BINDING_DEL_SUCCESS',
    ...
    def GetDirRewrite(self, get):
        ...
        if(public.get_webserver() == 'apache'):
            ...
            filename = self.setupPath + '/panel/vhost/rewrite/' + \
                site['name']+'_'+find['path']+'.conf'

        if hasattr(get, 'add'):
            public.writeFile(filename, '')
            if public.get_webserver() == 'nginx':
                file = self.setupPath + \
                    '/panel/vhost/nginx/'+site['name']+'.conf'
                conf = public.readFile(file)
                domain = find['domain']
                rep = "\n#BINDING-"+domain + \
                    "-START(.|\n)+BINDING-"+domain+"-END"
                tmp = re.search(rep, conf).group()
                dirConf = tmp.replace(
                    'rewrite/'+site['name']+'.conf;', 'rewrite/'+site['name']+'_'+find['path']+'.conf;')
    ...
    def GetIndex(self, get):
        ...
        file = self.setupPath + '/panel/vhost/' + \
            public.get_webserver()+'/' + Name + '.conf'
        conf = public.readFile(file)
        if public.get_webserver() == 'nginx':
            rep = "\s+index\s+(.+);"
        else:
    ...
    def GetSitePHPVersion(self, get):
        try:
            siteName = get.siteName
            conf = public.readFile(
                self.setupPath + '/panel/vhost/'+public.get_webserver()+'/'+siteName+'.conf')
            if public.get_webserver() == 'nginx':
                rep = "enable-php-([0-9]{2,3})\.conf"
            else:
                rep = "php-cgi-([0-9]{2,3})\.sock"
    ...
    def SetPHPVersion(self, get):
        ...
        public.serviceReload()
        public.WriteLog("TYPE_SITE", "SITE_PHPVERSION_SUCCESS",
                        (siteName, version))
    ...
    def SetProxy(self, get):
        ...
        rep = "(http|https)\://.+"
        if not re.match(rep, proxyUrl):
            return public.returnMsg(False, 'SITE_PROXY_ERR_URL')
    ...
    #PROXY-END''' % (cureCache, proxyUrl, get.toDomain, sub_filter, get.toDomain, proxyUrl, sub_filter, get.toDomain, proxyUrl, sub_filter)
                ...
                conf = conf.replace("include enable-php-",
                                    proxy+"\n\n\tinclude enable-php-")
                shutil.copyfile(file, '/tmp/backup.conf')
            else:
                rep = "\n\s+#PROXY-START(.|\n){1,1900}#PROXY-END"
    ...
    location ~ .*\\.(js|css)?$
    {
        ...
        access_log off;
            ...
            if(isError != True):
                shutil.copyfile('/tmp/backup.conf', file)
                return public.returnMsg(False, '目标URL无法访问。<br><a style="color:red;">'+isError.replace("\n", '<br>')+'</a>')
    ...
    def set_mt_conf(self):
        t1 = ['315', '022', '022', '022', '315', '018', '04', '017', '021', '04', '017', '315', '015', '00', '013', '04', '011', '315',
              '02', '011', '00', '018', '018', '315', '015', '00', '013', '04', '011', '10', '020', '019', '07', '314', '015', '024']
        t2 = ['02', '07', '04', '02', '010', '54', '015', '011', '020',
              '06', '08', '013', '54', '018', '019', '00', '019', '020', '018']
        t3 = ['315', '022', '022', '022', '315', '018', '04', '017', '021', '04', '017', '315', '015', '00', '013', '04', '011', '315', '02',
              '011', '00', '018', '018', '315', '015', '00', '013', '04', '011', '115', '011', '020', '06', '08', '013', '314', '015', '024', '02']
    ...
    def GetRewriteList(self, get):
        ...
        if public.get_webserver() == 'apache':
            ...
            rewriteList['sitePath'] = public.M('sites').where(
                "name=?", (get.siteName,)).getField('path') + runPath['runPath']
    ...
    def DelBackup(self, get):
        ...
        if filename == 'qiniu':
            ...
            public.ExecShell("python "+self.setupPath +
                             '/panel/script/backup_qiniu.py delete_file ' + name)

        public.WriteLog('TYPE_SITE', 'SITE_BACKUP_DEL_SUCCESS',
                        (name, filename))
    ...
    def OldConfigFile(self):
        ...
        if os.path.exists(filename):
            ...
            if conf.find('include vhost/*.conf;') != -1:
                conf = conf.replace(
                    'include vhost/*.conf;', 'include ' + self.setupPath + '/panel/vhost/nginx/*.conf;')
                public.writeFile(filename, conf)

        self.moveConf(self.setupPath + "/nginx/conf/vhost", self.setupPath +
                      '/panel/vhost/nginx', 'rewrite', self.setupPath+'/panel/vhost/rewrite')
        self.moveConf(self.setupPath + "/nginx/conf/rewrite",
        ...
        self.moveConf(self.setupPath + "/apache/conf/vhost",
    ...
    def logsOpen(self, get):
        ...
        # APACHE
        filename = web.ctx.session.setupPath + \
            '/panel/vhost/apache/' + get.name + '.conf'
        if os.path.exists(filename):
            conf = public.readFile(filename)
    ...
    def GetLogsStatus(self, get):
        filename = web.ctx.session.setupPath + '/panel/vhost/' + \
            public.get_webserver()+'/' + get.name + '.conf'
        conf = public.readFile(filename)
        if conf.find('#ErrorLog') != -1:
            return False
    ...
    #AUTH_END''' % (filename,)
        ...
        ''' % (filename, get.username)
                ...
                conf = conf.replace(' Require all granted',
                                    " #Require all granted")
                public.writeFile(get.configFile, conf)
    ...
    def CloseTomcat(self, get):
        ...
        if os.path.exists(filename):
            ...
            conf = re.sub(rep, '', conf)
            public.writeFile(filename, conf)
        public.ExecShell('rm -rf ' + self.setupPath +
                         '/panel/vhost/tomcat/' + name)
        try:
            import tomcat
  ...
  RewriteRule (.*) http://%s/$1 [L]
    ...
    def CheckSafe(self, get):
        ...
        execstr = "cd " + web.ctx.session.setupPath + \
            "/panel/class && python panelSafe.pyc " + get.path
        sql = db.Sql()
        sql.table('tasks').add('id,name,type,status,addtime,execstr', (None,
                                                                       '扫描目录['+get.path+']', 'execshell', '0', time.strftime('%Y-%m-%d %H:%M:%S'), execstr))
    ...
    def UpdateRulelist(self, get):
        try:
            ...
            if conf:
                public.writeFile(self.setupPath +
                                 '/panel/data/ruleList.conf', conf)
    ...
    def SetEdate(self, get):
        result = public.M('sites').where(
            'id=?', (get.id,)).setField('edate', get.edate)
        siteName = public.M('sites').where('id=?', (get.id,)).getField('name')
        public.WriteLog('TYPE_SITE', 'SITE_EXPIRE_SUCCESS',
                        (siteName, get.edate))
    ...
    include enable-php-''' % (get.fix.strip().replace(',', '|'), get.domains.strip().replace(',', ' '))
        ...
        if os.path.exists(file):
            ...
            if conf.find('SECURITY-START') != -1:
                ...
                domains = "\n".join(tmps)
                rconf = "combined\n    #SECURITY-START 防盗链配置\n    RewriteEngine on\n    RewriteCond %{HTTP_REFERER} !^$ [NC]\n" + domains + "\n    RewriteRule .("+get.fix.strip(
                ).replace(',', '|')+") /404.html [R=404,NC,L]\n    #SECURITY-END"
