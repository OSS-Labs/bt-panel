...
class ajax:
    ...
    def GetPHPStatus(self, get):
        ...
        result = public.httpGet(
            'http://127.0.0.1/phpfpm_'+version+'_status?json')
        ...
        for i in range(len(data)):
            ...
            for n in range(len(data[i]['versions'])):
                ...
                for task in tasks:
                    ...
                    tmp1 = tmp.split('-')
                    if data[i]['name'] == 'PHP':
                        if tmp1[0].lower() == data[i]['name'].lower() and tmp1[1] == data[i]['versions'][n]['version']:
                            isTask = task['status']
                    else:
                ...
                if data[i]['name'] == 'PHP':
                    ...
                    version = public.readFile(
                        web.ctx.session.rootPath+'/server/'+data[i]['name'].lower()+'/version.pl')
                    if not version:
                        continue
    ...
    def GetLibList(self, get):
        ...
        for i in range(len(data)):
            ...
            data[i]['optstr'] = self.GetLibOpt(
                data[i]['status'], data[i]['opt'])
        return data
    ...
    def GetLibOpt(self, status, libName):
        optStr = ''
        if status == '未安装':
            optStr = '<a class="link" href="javascript:InstallLib(\'' + \
                libName+'\');">安装</a>'
        else:
            libConfig = '配置'
            if(libName == 'beta'):
            ...
            optStr = '<a class="link" href="javascript:SetLibConfig(\''+libName+'\');">'+libConfig + \
    ...
    def SetQiniuAS(self, get):
        ...
        result = public.ExecShell(
            "python " + web.ctx.session.setupPath + "/panel/script/backup_"+get.name+".py list")

        if result[0].find("ERROR:") == -1:
            public.WriteLog("插件管理", "设置插件["+info['name']+"]AS!")
            return public.returnMsg(True, '设置成功！')
        return public.returnMsg(False, '无法连接到'+info['name']+'服务器，请检查[AK/SK/存储空间]设置是否正确！')
    ...
    def SetBeta(self, get):
        ...
        if data['status']:
            public.writeFile('data/beta.pl', get.bbs_name +
                             '|' + get.qq + '|' + get.email)
        return data
    # 取内测资格状态
    ...
    def GetNetWorkList(self, get):
        ...
        for netstat in netstats:
            ...
            del(tmp)
        networkList = sorted(
            networkList, key=lambda x: x['status'], reverse=True)
        return networkList
    ...
    def GetProcessList(self, get):
        ...
        processList = sorted(
            processList, key=lambda x: x['cpu_times'], reverse=True)
        return processList
    ...
    def KillProcess(self, get):
        ...
        name = p.name()
        if name == 'python':
            return public.returnMsg(False, 'KILL_PROCESS_ERR')
    ...
    def GoToProcess(self, name):
        ps = ['sftp-server', 'login', 'nm-dispatcher', 'irqbalance', 'qmgr', 'wpa_supplicant', 'lvmetad', 'auditd', 'master', 'dbus-daemon', 'tapdisk', 'sshd', 'init', 'ksoftirqd', 'kworker', 'kmpathd', 'kmpath_handlerd', 'python',
              'kdmflush', 'bioset', 'crond', 'kthreadd', 'migration', 'rcu_sched', 'kjournald', 'iptables', 'systemd', 'network', 'dhclient', 'systemd-journald', 'NetworkManager', 'systemd-logind', 'systemd-udevd', 'polkitd', 'tuned', 'rsyslogd']
    ...
    def GetNetWorkIo(self, get):
        # 取指定时间段的网络Io
        data = public.M('network').dbfile('system').where("addtime>=? AND addtime<=?", (get.start, get.end)).field(
    ...
    def GetDiskIo(self, get):
        # 取指定时间段的磁盘Io
        data = public.M('diskio').dbfile('system').where("addtime>=? AND addtime<=?", (get.start, get.end)).field(
    ...
    def GetCpuIo(self, get):
        ...
        data = public.M('cpuio').dbfile('system').where("addtime>=? AND addtime<=?",
                                                        (get.start, get.end)).field('id,pro,mem,addtime').order('id asc').select()
    ...
    def get_load_average(self, get):
        data = public.M('load_average').dbfile('system').where("addtime>=? AND addtime<=?", (
            get.start, get.end)).field('id,pro,one,five,fifteen,addtime').order('id asc').select()
        return self.ToAddtime(data)
    ...
    def UpdatePanel(self, get):
        ...
        try:
            ...
            if hasattr(web.ctx.session, 'updateInfo') == True and hasattr(get, 'check') == False:
                ...
                data['system'] = panelsys.GetSystemVersion() + '|' + str(mem.total / 1024 / 1024) + 'MB|' + public.getCpuType() + \
                    '*' + str(psutil.cpu_count()) + '|' + \
                    public.get_webserver() + '|' + web.ctx.session.version
                data['system'] += '||' + \
                    self.GetInstalleds(mplugin.getPluginList(None))
                data['logs'] = logs
                data['oem'] = ''
            ...
            if(updateInfo['force'] == True or hasattr(get, 'toUpdate') == True or os.path.exists('data/autoUpdate.pl') == True):
                ...
                if httpUrl:
                    updateInfo['downUrl'] = httpUrl + '/install/' + \
                        uptype + '/LinuxPanel-' + \
                        updateInfo['version'] + '.zip'
    ...
    def GetPHPInfo(self, get):
        ...
        phpinfo = public.httpGet(
            'http://127.0.0.2/' + get.version + '/phpinfo.php')
...
''' % (opt,)
    ...
    def setPHPMyAdmin(self, get):
        ...
        if hasattr(get, 'port'):
            ...
            get.id = public.M('firewall').where(
                'port=?', (oldPort,)).getField('id')
            get.port = oldPort
        ...
        if hasattr(get, 'phpversion'):
            if public.get_webserver() == 'nginx':
                ...
                conf = public.readFile(filename)
                rep = "php-cgi.*\.sock"
                conf = re.sub(rep, 'php-cgi-' +
                              get.phpversion + '.sock', conf, 1)
            else:
                rep = "php-cgi.*\.sock"
                conf = re.sub(rep, 'php-cgi-' +
                              get.phpversion + '.sock', conf, 1)
        ...
        if hasattr(get, 'status'):
            if conf.find(web.ctx.session.setupPath + '/stop') != -1:
                conf = conf.replace(
                    web.ctx.session.setupPath + '/stop', web.ctx.session.setupPath + '/phpmyadmin')
                msg = public.getMsg('START')
            else:
                conf = conf.replace(
                    web.ctx.session.setupPath + '/phpmyadmin', web.ctx.session.setupPath + '/stop')
    ...
    def GetWarning(self, get):
        ...
        if not os.path.exists(warningFile):
            return public.returnMsg(False, '警告列表不存在！')
        import json
        import time
