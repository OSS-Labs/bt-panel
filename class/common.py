...
class panelSetup:
    def __init__(self):
        ...
        if ua.find('spider') != -1 or ua.find('bot') != -1:
            raise web.redirect('https://www.baidu.com')
...
class panelAdmin(panelSetup):
    ...
    def checkAddressWhite(self):
        ...
        if not web.ctx.ip in token['address']:
            raise web.seeother('/login')
    ...
    def checkWebType(self):
        ...
        if os.path.exists(self.setupPath+'/'+web.ctx.session.webserver+'/version.pl'):
            web.ctx.session.webversion = public.readFile(
                self.setupPath+'/'+web.ctx.session.webserver+'/version.pl').strip()
