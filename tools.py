    ...
    try:
        ...
        if os.path.exists(filename):
            ...
            if version.find('2.2') != -1:
                print 'BT-Panel 3.x 不支持 Apache 2.2 ！';
                return;
    except:
        pass
...
def ClearMail():
    ...
    for d in os.listdir(rpath):
        ...
        for n in os.listdir(dpath):
            ...
            num += 1
        print '|-已完成 ['+dpath+'] 的清理，删除 ['+str(num)+'] 个文件，共释放磁盘空间 ['+ToSize(size)+'] ！';
        total += size;
        count += num;
    print '======================================================================='
    print '|-已完成 spool 的清理，删除 ['+str(count)+'] 个文件，共释放磁盘空间 ['+ToSize(total)+'] ！';
    return total,count
...
def ClearSession():
    ...
    for d in os.listdir(spath):
        ...
        print '\t\033[1;32m完成\033[0m'
        count += 1;
    print '|-已完成 php_session 的清理，删除 ['+str(count)+'] 个文件，共释放磁盘空间 ['+ToSize(total)+'] ！';
    return total,count
